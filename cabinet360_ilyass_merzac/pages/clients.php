...
try {
    ...
    if ($search) {
        ...
        $stmt->execute([$search_term, $search_term, $search_term, $search_term, $per_page, $offset]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM clients ORDER BY created_at DESC LIMIT ? OFFSET ?");
